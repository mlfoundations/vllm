...
class _RoutedExpertsCapturerReal(RoutedExpertsCapturer):
    ...
    def __init__(
        ...
        # ---- Async D2H pipeline ----
        # #237 (rank-symmetric epilogue): the D2H staging machinery
        # (_device_staging / _pinned_staging / _copy_stream / _copy_event) is
        # allocated on EVERY rank, not just rank 0. The per-step epilogue
        ...
        # buffer, but never scatters it into a host cache (it cannot perturb
        # what rank 0 captures). The extra per-rank cost is the staging buffers
        # (same size as rank 0's), accepted to guarantee op-stream symmetry.
    ...
    def sync_fwd_experts_buffer_DtoH(
        ...
        # #237 (rank-symmetric epilogue): the per-step D2H op sequence below is
        # issued IDENTICALLY on EVERY TP/CP rank -- the main-stream snapshot, the
        # copy-stream wait/copy/record, and the prior-copy event finalize all run
        # on every rank regardless of `host_cache`. Only the purely-host
        # bookkeeping (the host-cache scatter + the pending-copy metadata) is
        # rank-0-gated, because only rank 0 owns a host cache.
        #
        # WHY: before this change, non-rank-0 ranks early-returned on
        # `host_cache is None` while rank 0 alone issued the main-stream snapshot
        # copy + the copy-stream wait_stream(main_stream)/pinned copy/event ops.
        # Under enforce_eager (no CUDA graph to enforce identical op streams
        # across ranks) that rank-0-only main-stream op skewed rank 0's
        # collective launch timing vs ranks 1..N; on long decode the skew
        # desynced the group inside the forward -> execute_model hang -> 900s
        # watchdog -> EngineDead (job 923365). The fix equalizes the rank-0-only
        # work off the divergence path by making every rank issue the SAME
        # GPU/stream op set+order (the same op-stream symmetry the capture_routing
        # custom op already guarantees for the forward). The device snapshot stays
        # on the MAIN stream so it serializes before the next step's in-place MoE
        # buffer writes (anti-aliasing under async scheduling preserved).
        ...
        #    EVERY rank (every rank has the staging buffers/stream/event), so the
        #    op stream is identical across ranks. The main-stream snapshot
        #    serializes after this step's MoE writes (same stream) and is private
    ...
    def finalize_pending_copy(self):
        ...
        #237 (rank-symmetric epilogue): scatter is host-cache-gated (only rank 0
        has a host cache); non-rank-0 ranks now also carry a pending copy + event
        but have no host cache, so they only clear the event, never scatter. In
        practice the only caller (extract_routed_experts_for_current_batch) is
        already host_cache-gated, so non-rank-0 does not reach here -- this guard
        is defensive."""
        if self._has_pending_copy:
            self._copy_event.synchronize()
            if self.host_cache is not None:
