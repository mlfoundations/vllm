...
def set_device_control_env_var(
    ...
    # For Ray backend, nnodes is forced to 1 (Ray manages node discovery
    # internally), so local_world_size = world_size = TP*PP which can exceed
    # the actual number of GPUs on this node.  With multi-node PP + DP, each
    # DP rank's workers span remote nodes — local GPU partitioning is
    # meaningless because Ray actors already have correct device assignments.
    # Skip the CUDA_VISIBLE_DEVICES override entirely for Ray; the child
    # EngineCore inherits the current env and Ray handles the rest.
    ...
    try:
        value = ",".join(
            ...
            for i in range(
                ...
                (local_dp_rank + 1) * local_world_size,
            )
        )
